...
public class JwtTokenUtil {
   ...
   * @return Token
   */
  public Token generateToken(User user, DataSourceInfo dataSourceInfo) {
    Map<String, Object> claims = new HashMap<>();
    String roleName = userRepository.getRoleNameById(user.getRoleIdFk(), dataSourceInfo) ;
    String token = doGenerateToken(user.getEmail(), roleName,user.enableMFA());
   ...
   * @return token
   */
  private String doGenerateToken(String subject, String roleName, Boolean mfaStatus) {

    return Jwts.builder()
        .claim("Roles", roleName)
            .claim("mfaStatus",mfaStatus)
        .setSubject(subject)
