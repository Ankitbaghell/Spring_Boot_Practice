...
public class RdbmsUserRepository implements IUserRepository {
  ...
  private Function<Record, User> fromRecordToUser =
      record -> {
        return new User.Builder(
                ...
                record.get(Tables.TRS_USER.ROLE_ID_FK))
  ...
  private final Function<Record, User> fromRecordToUserDec =
      record -> {
        return new User.Builder(
                ...
                (String) record.get(Tables.TRS_USER.EMAIL), record.get(Tables.TRS_USER.ROLE_ID_FK))
   ...
   */
  @Override
  public String getRoleNameById(Long roleIdFk, DataSourceInfo dataSourceInfo) {
    try (Connection conn =
                 Datasource.getInstance(
                         ...
                         .getConnection()) {
      DSLContext dsl = DSL.using(conn);
      Record record = dsl.select().from(Tables.TRS_ROLE).where(Tables.TRS_ROLE.ID.eq(roleIdFk)).fetchOne();
      return record.get(Tables.TRS_ROLE.ROLE_NAME);
    } catch (Exception ex) {
      throw new RuntimeException(ex);
    }
  }
